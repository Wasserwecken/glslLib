...
float noise_value(float point, float seed, float smoothness)
{
    ...
    float interpolation;
    EasingPowerInOut(fract(point), smoothness, interpolation);
...
float noise_value(vec2 point, vec2 seed, float smoothness)
{
    ...
    vec2 interpolation;
    EasingPowerInOut(fract(point).x, smoothness, interpolation.x);
    EasingPowerInOut(fract(point).y, smoothness, interpolation.y);
...
float noise_value(vec3 point, vec3 seed, float smoothness)
{
    ...
    EasingPowerInOut(fract(point).x, smoothness, interpolation.x);
    EasingPowerInOut(fract(point).y, smoothness, interpolation.y);
    EasingPowerInOut(fract(point).z, smoothness, interpolation.z);
...
float noise_perlin(float point, float seed)
{
    ...
    point = fract(point);
    float interpolation;
    EasingSmooth(point, 2, interpolation);
...
float noise_perlin(vec2 point, vec2 seed)
{
    ...
    point = fract(point);
    vec2 interpolation;
    EasingSmooth(point.x, 2, interpolation.x);
    EasingSmooth(point.y, 2, interpolation.y);
...
float noise_perlin(vec3 point, vec3 seed)
{
    ...
    EasingSmooth(point.x, 2, interpolation.x);
    EasingSmooth(point.y, 2, interpolation.y);
    EasingSmooth(point.z, 2, interpolation.z);
...
float noise_voronoi_manhattan(vec2 point, vec2 seed)
{
    ...
    for(float x = -1.0; x < 2.0; x++)
    {
        for(float y = -1.0; y < 2.0; y++)
        {
            ...
            float currentDist;
            ManhattenLength(center - tile_pos + neighbour, currentDist);
